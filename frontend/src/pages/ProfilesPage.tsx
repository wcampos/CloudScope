    ...
    parseConfig.mutate(configText, {
      onSuccess: (created) => {
        if (created.length === 0) {
          ...
          notify(
            `${created.length} profiles imported: ${created.map((p) => p.name).join(", ")}`,
            "success"
          );
        }
      },
    ...
    return (
      <Container className="py-4">
        ...
        <div
          ...
          style={{ padding: "2rem", textAlign: "center", maxWidth: "32rem", margin: "0 auto" }}
        >
          ...
          {isNetworkError && (
            ...
            >
              ...
              <ul style={{ marginTop: "0.5rem", marginBottom: 0, paddingLeft: "1.25rem" }}>
                <li>
                  <strong>Dev (npm run dev):</strong> Run API on port 5001:{" "}
                  <code style={{ fontSize: "0.75rem" }}>
                    cd api && uvicorn main:app --host 0.0.0.0 --port 5001
                  </code>
                  . DB: <code style={{ fontSize: "0.75rem" }}>docker compose up -d db</code> then
                  set{" "}
                  <code style={{ fontSize: "0.75rem" }}>
                    DATABASE_URL=postgresql://cloudscope:cloudscope@localhost:5432/cloudscope
                  </code>{" "}
                  in <code style={{ fontSize: "0.75rem" }}>api/.env</code>.
                </li>
                <li>
                  <strong>Docker:</strong> Run{" "}
                  <code style={{ fontSize: "0.75rem" }}>docker compose up -d</code>, open{" "}
                  <strong>http://localhost:3000</strong> (not 5173). Check API:{" "}
                  <a
                    href="/health"
                    target="_blank"
            ...
            <div className="card-body">
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    border:
                      "1px solid " +
                      (addTab === "profile" ? "var(--cs-primary)" : "var(--cs-gray-300)"),
                ...
                <button
                  ...
                  style={{
                    ...
                    border:
                      "1px solid " +
                      (addTab === "paste" ? "var(--cs-primary)" : "var(--cs-gray-300)"),
                ...
                <button
                  ...
                  style={{
                    ...
                    border:
                      "1px solid " +
                      (addTab === "role" ? "var(--cs-primary)" : "var(--cs-gray-300)"),
              ...
              {addTab === "profile" && (
                <div key="panel-profile" style={{ paddingTop: "0.25rem" }}>
                  ...
                  >
                    Enter name, access key, secret key, and region. Optional session token for
              ...
              {addTab === "role" && (
                <div key="panel-role" data-panel="set-role" style={{ paddingTop: "0.25rem" }}>
                  ...
                  >
                    Pick an existing profile (its credentials are used) and add a new profile that
                    assumes an AWS role. No access keys here — only{" "}
                    <strong>Use existing role</strong> (role name) or{" "}
                    <strong>Custom role configuration</strong> (JSON).
                  </p>
                  <SetRoleForm
                    profiles={profiles}
              ...
              {addTab === "paste" && (
                <div key="panel-paste" style={{ paddingTop: "0.25rem" }}>
                  ...
                  >
                    Paste from <strong>~/.aws/credentials</strong> (access key, secret) or{" "}
                    <strong>~/.aws/config</strong> (role_arn, source_profile). Choose format below.
                  </p>
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border:
                          "1px solid " +
                          (pasteFormat === "config" ? "var(--cs-primary)" : "var(--cs-gray-300)"),
                        borderRadius: "var(--cs-radius)",
                  ...
                  {pasteFormat === "credentials" ? (
                    ...
                    <ConfigParser onSubmit={handleParseConfig} isLoading={parseConfig.isPending} />
                  )}
                </div>
              )}
            </div>
              ...
              {profiles.length === 0 ? (
                <div className="empty-state" style={{ padding: "2rem" }}>
                  ...
                  <p style={{ marginBottom: "1rem" }}>
                    Add your first AWS profile in the box on the left using one of these:
                  </p>
                  <ul
                    style={{
                      textAlign: "left",
                  ...
                  >
                    <li>
                      <strong>Add new profile</strong> — Form: name, access key, secret key, region
                      (no role).
                    </li>
                    <li>
                      <strong>Import by paste</strong> — Paste{" "}
                      <code style={{ fontSize: "0.8rem" }}>~/.aws/credentials</code> or{" "}
                      <code style={{ fontSize: "0.8rem" }}>~/.aws/config</code> to add profiles.
                    </li>
                    <li>
                      <strong>Set Role</strong> — Add an AWS role (existing or custom) using an
                      existing profile&apos;s credentials.
                    </li>
                  </ul>
                  <p
                    style={{
                  ...
                  >
                    If you just added or imported a profile, click <strong>Refresh</strong> above to
                    load the list.
                  </p>
                </div>
              ) : (
                  ...
                  if (listTab === "active" && filtered.length === 0) {
                    return (
                      <div className="empty-state" style={{ padding: "1.5rem" }}>
                        <p style={{ margin: 0, color: "var(--cs-gray-500)" }}>
                          No active profile. Select one and click Set as Active.
                        </p>
                      </div>
                    );
                  }
                    ...
                    <>
                      {filtered.map((profile) => (
                        <ProfileCard
                          ...
                          profile={profile}
                          isActive={
                            selectedId === profile.id || (selectedId == null && profile.is_active)
                          }
                      ...
                      >
                        Select a profile and click <strong>Use</strong> to set it as the active
